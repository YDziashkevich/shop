    ...
    <div id="content">
        ...
        if(!empty($orders)){
            foreach($orders as $keyOrder => $valueOrder){
                $head ="";
                $head .= "<h3>Номер заказа: " . $valueOrder["data"]["id"] . ". Дата заказа: ".$valueOrder["data"]["date"]." </h3>";
                $head .= "<h3>Имя заказчика: " . $valueOrder["data"]["nameUser"] . ". Телефон: ".$valueOrder["data"]["phone"]." </h3><br/>";
